...
router.post('/', async (req, res, next) => {
    ...
    try {
        const network = await dockerService.createNetwork({ Name: name, Driver: driver });
...
router.post('/:id/connect', async (req, res, next) => {
    ...
    try {
        ...
        logAction('network', `Container ${containerId} connected to network ${req.params.id}`, 'success');
        res.status(200).send({ message: 'Container connected successfully' });
    } catch (error) {
        logAction('network', `Error connecting container ${containerId} to network ${req.params.id}`, 'error');
        next(error);
    }
});
...
router.post('/:id/disconnect', async (req, res, next) => {
    ...
    try {
        ...
        logAction('network', `Container ${containerId} disconnected from network ${req.params.id}`, 'success');
        res.status(200).send({ message: 'Container disconnected successfully' });
    } catch (error) {
        logAction('network', `Error disconnecting container ${containerId} from network ${req.params.id}`, 'error');
        next(error);
    }
});
